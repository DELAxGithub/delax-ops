import os
import requests
import json
import base64
import sys
import subprocess
import csv

csv_path = "/Users/delaxpro/src/delax-ops/script/Case16_30fortTTS.csv"

# 1. 最初の3件（+異なる音声バリエーションを含むように抽出）のテキストを取得
lines_to_generate = []
with open(csv_path, "r", encoding="utf-8") as f:
    reader = csv.reader(f)
    header = next(reader)
    
    # 異なる声のサンプルを3つ選ぶ
    # カラム: Case(0), タイプ(1), テキスト(2), 出典/役割(3), 音声割り当て(4), 備考(5)
    seen_voices = set()
    for row in reader:
        if len(row) > 4:
            text = row[2]
            voice_type = row[4]
            # 若手女性、老年男性A、中年男性などをばらけさせて取得する
            if voice_type not in seen_voices and voice_type != "":
                # Ensure we capture a female voice for variety
                if len(lines_to_generate) == 2 and "若手女性" not in seen_voices and voice_type != "若手女性":
                    continue
                lines_to_generate.append({"text": text, "voice_assigned": voice_type})
                seen_voices.add(voice_type)
        if len(lines_to_generate) == 3:
            break

# Gemini TTSの声のマッピングの想定（ドキュメントに詳細指定がないため独自想定）
# PUCK (男性, 若め), CHARON (男性, 渋め), KORE (女性, 落ち着き), FENRIR (男性, 低め), AOEDE (女性, 柔らかめ)
voice_map = {
    "老年男性A": "Charon",
    "老年男性B": "Fenrir",
    "若手女性": "Kore",
    "中年男性": "Charon",
    "中年女性": "Aoede",
    "若手男性": "Puck",
}

# 2. GEMINI_API_KEYの取得
api_key = os.environ.get("GEMINI_API_KEY")
if not api_key:
    env_path = "/Users/delaxpro/src/delax-ops/ops/media/orion/.env"
    if os.path.exists(env_path):
        with open(env_path, encoding="utf-8") as env_f:
            for eline in env_f:
                if eline.startswith("GEMINI_API_KEY="):
                    api_key = eline.strip().split("=", 1)[1].strip('"\'')
                    break

if not api_key:
    print("Error: No GEMINI_API_KEY found in environment or .env file.")
    sys.exit(1)

# 3. Gemini TTS APIの呼び出し
url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-tts:generateContent?key={api_key}"

for i, item in enumerate(lines_to_generate):
    text = item["text"]
    voice_assigned = item["voice_assigned"]
    gemini_voice = voice_map.get(voice_assigned, "Charon") # デフォルトはCharon
    
    print(f"[{i+1}/3] Generating TTS for: {text} | Voice: {voice_assigned} -> {gemini_voice}")
    
    payload = {
        "contents": [
            {
                "parts": [
                    {"text": text}
                ]
            }
        ],
        "generationConfig": {
            "responseModalities": ["AUDIO"],
            "speechConfig": {
                "voiceConfig": {
                    "prebuiltVoiceConfig": {"voiceName": gemini_voice}
                }
            }
        }
    }
    
    resp = requests.post(url, json=payload)
    if resp.status_code != 200:
        print("API Error:", resp.status_code, resp.text)
        continue
        
    data = resp.json()
    try:
        b64data = data["candidates"][0]["content"]["parts"][0]["inlineData"]["data"]
        pcm_bytes = base64.b64decode(b64data)
        
        mp3_name = f"/Users/delaxpro/src/delax-ops/script/sample_{voice_assigned}.mp3"
        
        # ffmpegでs16le(PCM)からMP3へ変換。アテンポ(0.9)で少しゆっくり
        cmd = [
            "ffmpeg", "-y", "-f", "s16le", "-ar", "24000", "-ac", "1", "-i", "-",
            "-filter:a", "atempo=0.9", mp3_name
        ]
        proc = subprocess.run(cmd, input=pcm_bytes, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        
        if proc.returncode == 0:
            print(f" -> Saved {mp3_name}")
        else:
            print(f" -> ffmpeg error: {proc.stderr.decode('utf-8')}")
            
    except Exception as e:
        print("Error parsing response or saving file:", e)

print("Done.")
